...
A[3] = function(icon, isMulti)
    ...
    local useFlamestrike = Player:GetDeBuffsUnitCount(A.Ignite.ID) >= 3 and MultiUnits:GetActiveEnemies() >= 3
    ...
    local function EnemyRotation(unitID)
        ...
        if A.IceFloes:IsReady(player, nil, nil, true) then
            ...
            if (Unit(player):IsCasting() == A.Pyroblast:Info() or Unit(player):IsCasting() == A.ShiftingPower:Info()) and Unit(player):HasBuffs(A.IceFloes.ID) <= A.Pyroblast:GetSpellCastTime() then
                return A.IceFloes:Show(icon)
            end
        end
        ...
        if not hasCombustion then
            ...
            if A.Flamestrike:IsReady(player, nil, nil, true) and useFlamestrike and hotStreak and not Unit(unitID):IsBoss() and Unit(player):IsCastingRemains() < 0.5 then
                return A.Flamestrike:Show(icon)
            end

            --Hardcast
            if A.Pyroblast:IsReady(unitID, nil, nil, true) and (hasBlessing or hasPyroclasm) and (not isMoving or hasFloes) and not hotStreak and Unit(unitID):TimeToDie() > 15 and Unit(player):IsCastingRemains() < 0.5 and Unit(player):IsCasting() ~= A.Pyroblast:Info() then
                return A.Pyroblast:Show(icon)
            end
        ...
        if hasCombustion then
            ...
            --Hardcast
            if A.Pyroblast:IsReady(unitID, nil, nil, true) and (hasPyroclasm or hasBlessing) and Unit(unitID):TimeToDie() > 15 and Unit(player):IsCastingRemains() < 0.5 and Unit(player):IsCasting() ~= A.Pyroblast:Info() then
                return A.Pyroblast:Show(icon)
            end

            if A.Flamestrike:IsReady(player, nil, nil, true) and (hotStreak or (heatingUp and Unit(player):IsCasting() == A.Scorch:Info())) and Unit(player):IsCastingRemains() < 0.5 then
                return A.Flamestrike:Show(icon)
            end

            --Instant
            if A.Pyroblast:IsReady(unitID, nil, nil, true) and (hotStreak or (heatingUp and Unit(player):IsCasting() == A.Scorch:Info())) and Unit(player):IsCastingRemains() < 0.5 then
                return A.Pyroblast:Show(icon)
            end
