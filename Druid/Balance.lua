...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unitID)
		...
		local spendAP = futureAstralPower >= 85 or (starlordStacks > 0 and starlordStacks < 3) or not A.Starlord:IsTalentLearned()
		...
		if Unit(player):IsCastingRemains() < 0.5 and A.Moonfire:IsInRange(unitID) then
			...
			if A.AstralCommunion:IsReady(player, nil, nil, true) and astralPowerDeficit >= 70 then
